...
export const playableCash = (cashPaise: number, monthlyRupees: number, active: boolean) =>
  cashPaise + (active ? monthlyRupees * 100 : 0);

export const executionCash = (openingPaise: number, trades: Trade[], stocks: Stock[]) =>
  trades.reduce((cash, trade) => {
    ...
    const movement = stock.close_paise * trade.shares * (trade.side === "SELL" ? 1 : -1);
